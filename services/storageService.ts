...
export class StorageService {
  ...
  static async uploadImageWithPreview(
    ...
    try {
      ...
      const storageRef = ref(
        rdTechStorage,
        `${storagePaths.assets}${finalFileName}`,
      );
      ...
      if (error.code === "storage/unauthorized") {
        throw new FirebaseError(
          "You don't have permission to upload files. Please check your authentication and try again.",
        );
  ...
  static async uploadPDF(file: File, fileName?: string): Promise<string> {
    try {
      ...
      const storageRef = ref(
        rdTechStorage,
        `${storagePaths.pdfs}${finalFileName}`,
      );
  ...
  static async uploadMultipleAssets(files: File[]): Promise<string[]> {
    try {
      const uploadPromises = files.map((file) =>
        StorageService.uploadAsset(file),
      );
  ...
  static async uploadMultipleImagesWithPreview(
    files: File[],
  ): Promise<{ url: string; preview: string }[]> {
    try {
      const uploadPromises = files.map((file) =>
        StorageService.uploadImageWithPreview(file),
      );
      return await Promise.all(uploadPromises);
    } catch (error) {
