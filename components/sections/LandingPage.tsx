...
export default function LandingPage() {
  ...
  const getImageGridClasses = (count: number) => {
    ...
    return (
      gridClasses[Math.min(count, 8) as keyof typeof gridClasses] ||
      gridClasses[8]
    );
  };
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        ...
        <motion.div
          className="absolute top-20 left-12 text-primary/60"
          animate={{ y: [0, 20, 0] }}
        ...
        <motion.div
          className="absolute bottom-28 right-16 text-accent/60"
          animate={{ y: [0, -20, 0] }}
        ...
        <motion.div
          className="absolute top-1/3 right-1/4 text-primary/50"
          animate={{ y: [0, 15, 0] }}
        ...
        <motion.div
          className="absolute bottom-16 left-1/3 text-accent/50"
          animate={{ y: [0, 25, 0] }}
      ...
      <div className="relative z-10 w-full flex flex-col items-center justify-center text-center px-4 sm:px-6 lg:px-8 py-24 gap-8">
        ...
        {imageUrls.length > 0 && (
          <motion.div ref={imagesRef} className="hero-images w-full">
            <div
              className={`grid ${getImageGridClasses(imageUrls.length)} mx-auto`}
            >
